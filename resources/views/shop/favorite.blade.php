            ...
            @foreach($shops as $shop)
                <div class="product-item">
                    ...
                    <div class="product-item-header">
                        @if($shop->photo ? $shop->photo->pluck('category')->first() == 'video' : false)
                            <video style="width: 225px; height: 200px;" controls="controls">
                                <source src="{{$shop->photo == null ? '' : $shop->photo->pluck('src')->first()}}">
                            </video>
                        @else
                            <img src="{{$shop->photo == null ? '' : $shop->photo->pluck('src')->first()}}" onclick="location.href = '{{route('shop.products', $shop->id)}}';" alt="" style="width: 100%;">
                        @endif

                    </div>
                    <div class="product-item-content">
                        <h3 class="item-title">{{$shop->title}}</h3>
